...
> text, email, password, number, date, color, checkbox, radio, file, submit, reset, button, image, range, tel, url, search, time, month, week, datetime-local, hidden

3. Mention at least four attributes of an input element

> type, name, value, placeholder, required, disabled, readonly, autofocus, autocomplete, min, max, step, pattern, multiple, accept, form, formaction, formenctype, formmethod, formnovalidate, formtarget, height, width, list, max, min, multiple, pattern, size, src, step, value

4. What is the importance of htmlFor?

> htmlFor is used to associate a label with an input element.
...
> A controlled input is an input whose value is controlled by React.

7. What do you need to write a controlled input?

> You need to write an event handler for every stateful update you need to make.
...
> onChange is used to listen for changes on an input field. onBlur is used to listen for changes on an input field when the input field loses focus. onSubmit is used to listen for changes on a form when the form is submitted.
...
> You bind data in React by using the value attribute on form elements. You also need to add an onChange event handler that updates the state whenever the input value changes.

13. What is validation?

> Validation is the process of checking if the data entered by the user is correct.
