...
const challenges = [
  ...
  },
  {
    ...
    slug: "javascript",
    url: "https://github.com/bishal/30-Days-Of-JavaScript",
    author: {
      firstName: "bishal",
      lastName: "karki",
    },
  },
  {
    ...
    slug: "react",
    url: "https://github.com/bishal/30-Days-Of-React",
    author: {
      firstName: "bishal",
      lastName: "karki",
    },
    ...
    return (
      <Router>
        <div className="App">
          ...
          <Prompt
            message={({ pathname }) => {
              return this.state.isLoggedIn && pathname.includes("/user/bishal")
...
> The default export is `BrowserRouter`

3. What is the use of the following Components(Route, NavLink, Switch, Redirect, Prompt)

> `Route` is used to render a component based on the path. `NavLink` is used to create a link to a specific route. `Switch` is used to render the first route that matches the path. `Redirect` is used to redirect the user to a specific route. `Prompt` is used to show a message to the user before they leave the page.

4. What is the use of the following props in Route component?

> `path` is used to specify the path of the route. `component` is used to specify the component to be rendered. `exact` is used to specify that the route should be rendered only if the path matches exactly. `strict` is used to specify that the route should be rendered only if the path matches exactly. `sensitive` is used to specify that the route should be rendered only if the path matches exactly.

5. What is the use of the following props in NavLink component?

> `to` is used to specify the path of the route. `activeClassName` is used to specify the class name to be applied to the link when the route is active. `activeStyle` is used to specify the style to be applied to the link when the route is active. `exact` is used to specify that the route should be rendered only if the path matches exactly. `strict` is used to specify that the route should be rendered only if the path matches exactly. `sensitive` is used to specify that the route should be rendered only if the path matches exactly.

6. What is the use of the following props in Redirect component?

> `to` is used to specify the path of the route. `push` is used to specify that the route should be pushed to the history stack.

7. What is the use of the following props in Prompt component?

> `message` is used to specify the message to be shown to the user before they leave the page.

8. What is the use of the following props in Switch component?

> `Switch` is used to render the first route that matches the path.

9. What is the use of the following props in BrowserRouter component?

> `basename` is used to specify the base path of the application. `forceRefresh` is used to specify that the page should be refreshed when the route changes. `getUserConfirmation` is used to specify the function to be called when the user tries to leave the page. `keyLength` is used to specify the length of the key to be used for the history stack.

10. What is the use of the following props in HashRouter component?

> `basename` is used to specify the base path of the application. `getUserConfirmation` is used to specify the function to be called when the user tries to leave the page. `hashType` is used to specify the type of hash to be used.

11. What is the use of the following props in MemoryRouter component?

> `initialEntries` is used to specify the initial entries of the history stack. `initialIndex` is used to specify the initial index of the history stack. `getUserConfirmation` is used to specify the function to be called when the user tries to leave the page. `keyLength` is used to specify the length of the key to be used for the history stack.

12. What is the use of the following props in StaticRouter component?

> `basename` is used to specify the base path of the application. `location` is used to specify the location of the route. `context` is used to specify the context of the route.

13. What is the use of the following props in Link component?

> `to` is used to specify the path of the route. `replace` is used to specify that the route should be replaced in the history stack.

14. What is the use of the following props in NavLink component?
